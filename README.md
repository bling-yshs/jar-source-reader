...
读取已存在的 sources jar 不依赖 IDE，主要基于本地文件系统操作；`fuzzy` 模式调用 Maven / Gradle 获取依赖列表时，构建工具可能按项目配置解析尚未缓存的插件或依赖。

## 🔎 Fuzzy 模式

当 AI 只知道类名、不知道目标类属于哪个依赖时，可以使用 `fuzzy` 模式自动定位来源 jar。工具会利用 Maven / Gradle 的原生能力遍历当前项目的依赖 jar 包，从中查找目标 Class 文件，再反推出对应的 sources jar。具体流程如下：
...
| 构建工具 | 获取依赖列表的难度 | 说明 |
|:---:|:---:|:---|
| **Maven** | 🟢 相对容易 | 通过 `mvnw dependency:build-classpath` 或 `mvn dependency:build-classpath` 获取 |
| **Gradle** | 🔴 较为困难 | 通过随工具附带的 init script 注入 `printAllJar` 任务获取 |
